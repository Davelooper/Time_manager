<template>
  ...
  <div class="flex flex-row gap-4">
    <Input type="text" :value="props.data.user.username" label="Username" placeholder="Your username" :onClick="saveUsername" />
    <Input type="text" :value="props.data.user.email" label="Email" placeholder="Your Email" :onClick="saveEmail"/>
  </div>
  <Input type="text" value="***********" label="Password" placeholder="Your Password" :disabled="!props.data || (props.data.id !== id)"/>
  <div class="flex flex-row gap-4">
    <Input type="select" :value="props.data.user.role" label="Role" :onClick="saveRole" :disabled="role !== 'admin'">
...
const saveUsername = async (username: string) => {
  try {
    await axios.put(`http://localhost:4000/api/users/${props.data.user.id}`, { user:{username} }, {
      headers: {
...
const saveEmail = async (email: string) => {
  try {
    await axios.put(`http://localhost:4000/api/users/${props.data.user.id}`, {user: {email} }, {
      headers: {
        Authorization: `Bearer ${getToken()}`,
      },
    });
...
const saveRole = async (role: string) => {
  try {
    await axios.put(`http://localhost:4000/api/users/${props.data.user.id}`, { user:{role} }, {
      headers: {
        Authorization: `Bearer ${getToken()}`,
      },
    });
