defmodule TimeManager.Account do
  ...
  def generateUserToken(user) do
    case Token.generate(%{"id" => user.id, "email" => user.email}) do
      {:ok, token} ->
        {:ok, token}
  ...
  @doc """
  Récupère l'utilisateur associé au token JWT fourni.
  Retourne `{:ok, user}` si le token est valide, sinon `:error`.
  """
  def get_user_by_jwt(token) do
  ...
  Utilise la bibliothèque Bcrypt pour la comparaison.
  """
  def verify_password(%User{hashed_password: hashed_password}, password) do
    Bcrypt.verify_pass(password, hashed_password)
  end

  @doc """
  Vérifie si le mot de passe donné est correct pour l'utilisateur avec l'email fourni.
  Retourne `{:ok, user}` si l'email et le mot de passe sont corrects, sinon `:error`.
  """
  ...
  def getUserFromMailAndUsername(email, username) do
